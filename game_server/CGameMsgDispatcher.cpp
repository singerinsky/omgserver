...
void CGameMsgDispatcher::dispatch_msg() {
	...
	if (event != NULL) {
		...
		case MSG_TYPE_CLIENT_CHANGE_FORMATION: {
			VLOG(1) << "客户端换阵型卡!!!";
			MsgClientChangeFormation* data =
					(MsgClientChangeFormation*) event->_msg_base;
			int rst = VerifyOperate(data->mid, data->cid,
					MSG_TYPE_CLIENT_CHANGE_FORMATION, data->time, public_key,
					data->code);
			if (rst != 0) {
				VLOG(1) << "未验证通过的操作！！！";
			} else {
		...
		case MSG_TYPE_CLIENT_CHANGE_MOVE: {
			...
			int rst = VerifyOperate(msg->mid, msg->cid,
					MSG_TYPE_CLIENT_CHANGE_MOVE, msg->time, public_key,
					msg->code);
			if (rst != 0) {
				VLOG(1) << "未验证通过的操作!!";
			} else {
		...
		case MSG_TYPE_CLIENT_CHANGE_PLAYER: {
			VLOG(1) << "客户端换人";
			MsgClientChangePlayer* msg =
					(MsgClientChangePlayer*) event->_msg_base;
			int rst = VerifyOperate(msg->mid, msg->cid,
					MSG_TYPE_CLIENT_CHANGE_PLAYER, msg->time, public_key,
					msg->code);
			if (rst != 0) {
				VLOG(1) << "未验证通过的操作!!";
			} else {
				...
				if (match != NULL) {
					VLOG(1) << "换人。。。。。";
					match->set_new_formation(msg->cid,
							msg->new_formation_pos_map_pid, msg->new_suitable);
				}
			}
			delete[] (char*) msg;
			event->_msg_base = NULL;
		...
		case MSG_TYPE_CLIENT_COACH_SPEAK: {
			...
			int rst = VerifyOperate(msg->mid, msg->cid,
					MSG_TYPE_CLIENT_COACH_SPEAK, msg->time, public_key,
					msg->code);
			if (rst != 0) {
		...
		case MSG_TYPE_CLIENT_TERMINAL_MATCH: {
			VLOG(1) << "客户端请求快速结束比赛!";
			MsgClientTerminalMatch* msg =
					(MsgClientTerminalMatch*) (event->_msg_base);
			int rst = VerifyOperate(msg->mid, msg->cid,
					MSG_TYPE_CLIENT_TERMINAL_MATCH, msg->time, public_key,
					msg->code);
			if (rst != 0) {
				VLOG(1) << "未验证通过的操作!!";
			} else {
		...
		case MSG_TYPE_CLIENT_MATCHING_SUCCESS: {
			MsgServerMatchingSuccess* msg =
					(MsgServerMatchingSuccess*) (event->_msg_base);
			//生成key，记录gate的信息
			...
			snprintf(match_info->key, 30, "%d_%d_%d", wait_id, msg->home_uid,
					msg->away_uid);
			CMatchManager::GetInstance()->AddArenaWaitPair(match_info->key,
					match_info);
		...
		case MSG_TYPE_CLIENT_PVN_MATCH_START:
		{
			...
			if(match != NULL) {
				...
				if(conn && (conn->connected())) {
					LOG(INFO)<<"Add gate server"<<conn->get_connection_id()<<" to match";
					match->add_listener_list(conn->get_connection_id(),conn);
					//发送比赛信息
...
MsgBase* CGameMsgDispatcher::CompressMsg(MsgBase* msgbase) {
	...
	int rst = compress(buffer + sizeof(MsgBase), &len_after_compress,
			(((unsigned char*) msgbase) + sizeof(MsgBase)),
			msgbase->msg_size - sizeof(MsgBase));
	if (rst < 0) {
...
int CGameMsgDispatcher::VerifyChangePlayerOperate(int mid, int cid, int pid,
		int operate_type, int zeit_time, const char* key, const char* code) {
...
int CGameMsgDispatcher::VerifyOperate(int mid, int cid, int operate_type,
		int zeit_time, const char* key, const char* code) {
	//md5($zeit.md5($public_key.$operate_type))
	std::ostringstream str;
	str << public_key;
	str << operate_type;
